...
export default function App() {
  ...
  useEffect(() => {
    ...
    const collectionPath = collection(db, "artifacts", appId, "public", "data", "training_events");
  ...
  const saveEvent = async (e) => {
    ...
    try {
      if (savingEvent) {
        await updateDoc(doc(db, "artifacts", appId, "public", "data", "training_events", savingEvent.id), savingFormData);
      } else {
  ...
  const deleteEvent = async () => {
    ...
    try {
      await deleteDoc(doc(db, "artifacts", appId, "public", "data", "training_events", editingEvent.id));
      closeModal();
    } catch (err) {
  ...
  const seedInitialData = async () => {
    ...
    const initialData = [
      ...
      { date: "2026-05-07", title: "メンタルサポートプログラム", type: "important" },
      { date: "2026-05-08", title: "通常研修", type: "normal" },
      { date: "2026-05-11", title: "入店研修・準備サポート", type: "important" },
      { date: "2026-05-12", title: "入店研修・準備サポート", type: "important" },
      ...
      { date: "2026-05-20", title: "合同新入社員研修(終日)", type: "important" },
      { date: "2026-05-21", title: "通常研修", type: "normal" },
      { date: "2026-05-22", title: "通常研修", type: "normal" },
      { date: "2026-05-23", title: "ラキャルプフェス・交流会", type: "event", description: "参加：久保さん、新妻さん" },
      { date: "2026-05-24", title: "ラキャルプフェス・交流会", type: "event", description: "参加：上野さん、金谷さん" },
  ...
  const getEventColors = (type) => {
    switch (type) {
      ...
      case "off":       return "bg-emerald-100 text-emerald-800 border-emerald-200";
      ...
      <div className="flex h-screen items-center justify-center bg-slate-50">
        <div className="bg-white rounded-2xl shadow-lg p-10 flex flex-col items-center gap-6 max-w-sm w-full mx-4">
          ...
          <div className="text-center">
            <h1 className="text-2xl font-bold text-indigo-700">研修スケジュール</h1>
            <p className="text-slate-500 mt-2 text-sm">Googleアカウントでログインしてください</p>
          </div>
          <button
    ...
    return (
      <div className="flex h-screen items-center justify-center bg-slate-50">
        <div className="bg-white rounded-2xl shadow-lg p-10 flex flex-col items-center gap-6 max-w-sm w-full mx-4 text-center">
          ...
          <div>
            <h2 className="text-xl font-bold text-slate-800">アクセス権がありません</h2>
            <p className="text-slate-500 mt-2 text-sm">{user.email} はアクセスが許可されていません。</p>
          </div>
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans">
      ...
      <header className="bg-indigo-600 text-white shadow-md">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex flex-col sm:flex-row justify-between items-center gap-4">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl sm:text-2xl font-bold">研修スケジュール共有アプリ</h1>
          </div>
          <div className="flex items-center gap-3">
            <span className="text-sm text-indigo-100 hidden sm:block">{user.displayName}</span>
            <button
              onClick={() => openModal()}
        ...
        <section className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {[
              ...
              { time: "お昼",  title: "休憩（1時間）",  sub: "※研修内容により変動" },
              { time: "17:00", title: "日報作成",       sub: "〜17:30" },
              { time: "17:30", title: "復習まとめ",      sub: "〜17:45" },
            ].map(({ time, title, sub }) => (
              <div key={time} className="flex items-start gap-3 p-3 bg-slate-50 rounded-lg">
                <span className="font-bold text-indigo-600 w-14 shrink-0 text-right">{time}</span>
                <div>
            ...
            <div className="flex items-start gap-3 p-3 bg-indigo-50 border border-indigo-100 rounded-lg">
              <span className="font-bold text-indigo-600 w-14 shrink-0 text-right">週1回</span>
              <div>
        ...
        <section className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
          ...
          {events.length === 0 && !eventsLoading && (
            <div className="p-8 text-center bg-yellow-50 border-b border-yellow-100">
              <p className="mb-4 text-yellow-800">まだ予定が登録されていません。</p>
              <button
          ...
          <div className="flex flex-col md:grid md:grid-cols-7 gap-[1px] bg-slate-200">
            {weekDays.map((wd, i) => (
              <div key={wd} className={`hidden md:block py-2 text-center text-sm font-bold bg-slate-100 ${i === 0 ? "text-red-600" : i === 6 ? "text-blue-600" : "text-slate-600"}`}>
                {wd}
              </div>
            ))}

            {calendarCells.map((cell) => {
              if (cell.type === "empty") return <div key={cell.key} className="hidden md:block bg-slate-50 min-h-[120px]" />;
                ...
                >
                  <div className="flex items-center justify-between md:mb-1">
                    <div className="flex items-baseline gap-1">
                      <span className={`font-bold text-lg md:text-base ${dayOfWeek === 0 ? "text-red-600" : dayOfWeek === 6 ? "text-blue-600" : "text-slate-800"}`}>{date}</span>
                      <span className="md:hidden text-xs font-medium text-slate-500">{weekDays[dayOfWeek]}曜</span>
                    </div>
                    <button className="hidden md:flex opacity-0 group-hover:opacity-100 items-center justify-center h-6 w-6 rounded-full bg-indigo-100 text-indigo-600 hover:bg-indigo-200 transition-all" title="予定を追加">
                      <Plus className="h-4 w-4" />
                    </button>
                    <button className="md:hidden text-indigo-600 p-1"><Plus className="h-5 w-5" /></button>
                  </div>
      ...
      {/* 予定 追加/編集モーダル */}
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4 z-50" onClick={closeModal}>
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <div className="flex justify-between items-center p-4 border-b border-slate-100 bg-slate-50">
              <h3 className="text-lg font-bold text-slate-800">{editingEvent ? "予定を編集" : "予定を追加"}</h3>
            ...
            <form onSubmit={saveEvent} className="p-6 space-y-4">
              ...
              <div>
                ...
                <input type="text" required placeholder="例：通常研修、新入社員歓迎会" value={formData.title} onChange={(e) => setFormData({ ...formData, title: e.target.value })}
                  className="w-full rounded-md border border-slate-300 px-3 py-2 text-slate-900 focus:outline-none focus:ring-2 focus:ring-indigo-500" />
              </div>
              <div>
              ...
              <div className="pt-4 flex items-center justify-between gap-3 border-t border-slate-100">
                {editingEvent ? (
                  <button type="button" onClick={deleteEvent} className="text-red-600 hover:text-red-800 hover:bg-red-50 px-3 py-2 rounded-md text-sm font-medium flex items-center gap-1 transition-colors">
                ...
                <div className="flex gap-2">
                  ...
                  <button type="submit" className="px-4 py-2 text-sm font-medium text-white bg-indigo-600 rounded-md hover:bg-indigo-700 transition-colors shadow-sm">
                    保存する
                  </button>
                </div>
              </div>
      ...
      {/* メンバー管理モーダル */}
      {isAdminOpen && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4 z-50" onClick={() => setIsAdminOpen(false)}>
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <div className="flex justify-between items-center p-4 border-b border-slate-100 bg-slate-50">
              <h3 className="text-lg font-bold text-slate-800 flex items-center gap-2">
                <Settings className="h-5 w-5 text-indigo-500" /> メンバー管理
              </h3>
              <button onClick={() => setIsAdminOpen(false)} className="text-slate-400 hover:text-slate-600 p-1 rounded-full hover:bg-slate-200 transition-colors">
                <X className="h-5 w-5" />
              </button>
            </div>
            ...
            <div className="p-6 space-y-5">
              ...
              {/* 現在の許可リスト */}
              <div>
                <p className="text-xs font-medium text-slate-500 mb-2">アクセス許可中 ({allowedEmails.length}名)</p>
                <ul className="space-y-1.5 max-h-64 overflow-y-auto">
                  {allowedEmails.map((email) => (
                    <li key={email} className="flex items-center justify-between px-3 py-2 rounded-lg bg-slate-50 text-sm">
                      <span className={`text-slate-700 truncate ${email === user.email ? "font-semibold" : ""}`}>
                        {email} {email === user.email && <span className="text-xs text-indigo-500 ml-1">（自分）</span>}
                      </span>
                      {email !== user.email && (
                        <button
